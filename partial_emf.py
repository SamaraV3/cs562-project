...
#this updated version has the logic for handling emf in setting up the f vector and should have the correct way of checking emf in check conditions
#still have to figure out how to pass the correct entry into the check conditions so that the emf can access that
import re
cur = [
    {'cust': 'Alice', 'prod': 'Widget', 'day': 1, 'month': 1, 'year': 2023, 'state': 'NY', 'quant': 100, 'date': '2023-01-01'},
    {'cust': 'Alice', 'prod': 'Widget', 'day': 2, 'month': 1, 'year': 2023, 'state': 'NJ', 'quant': 90, 'date': '2023-01-02'},
    ...
    {'cust': 'Charlie', 'prod': 'Widget', 'day': 2, 'month': 1, 'year': 2023, 'state': 'NY', 'quant': 120, 'date': '2023-01-02'},
    {'cust': 'Charlie', 'prod': 'Gadget', 'day': 3, 'month': 1, 'year': 2023, 'state': 'NJ', 'quant': 90, 'date': '2023-01-03'},
    {'cust': 'David', 'prod': 'Widget', 'day': 1, 'month': 1, 'year': 2023, 'state': 'NY', 'quant': 60, 'date': '2023-01-01'},
    {'cust': 'David', 'prod': 'Widget', 'day': 2, 'month': 1, 'year': 2023, 'state': 'NJ', 'quant': 180, 'date': '2023-01-02'},
]
S = ["cust", "1_sum_quant", "1_avg_quant", "2_count_prod", "2_sum_quant"]
n = 2  # 2 grouping variables
V = ["cust"]  # GROUP BY cust
F = [["1_sum_quant"], ["2_count_prod", "2_sum_quant"]]  # Aggregates for each GV
o = [
    ["1.state='NY'"],  # GV1: Only NY transactions
    ["2.state='NJ'", "2.quant>1_avg_quant"]  # GV2: NJ transactions where quant > avg from GV1
]
G = "2_count_prod > 0" 
...
# logic for determining this can be parsing through pred-list and cecking if the right side has format of agg_#_col
# disticntion may not need to be made if the changes are done within in the check conditon
